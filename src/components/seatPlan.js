    ...
    <div className='flex flex-col items-center'>
      <div style={{ width: "100%" }}>
        ...
        {movieSession && (
          ...
          >
            <strong className="session-movie-title">{movie.title}</strong> <br />
      ...
      <div className='CinemaPlan'>
        ...
        <p className='info mb-2 text-sm md:text-sm lg:text-base'>
          ...
          {selectedSeats.length > 0 && (
            <>
              for the price of{' '}
              <span className='total font-semibold'>Rp.{totalPrice.toLocaleString('id-ID')}</span>
            </>
          )}
        </p>

        {alertMsg && (
          <div className="alert-max-seat">
            {alertMsg}
          </div>
        )}
        ...
        <div style={{ textAlign: "center" }}>
          <button
            className={`buy-btn${selectedSeats.length === 0 ? ' disabled' : ''}`}
